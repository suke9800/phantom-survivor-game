...
- Implemented the previous title upgrade pass. Generated and integrated a logo, cute monster atlas, and expanded terrain atlas. Runtime monster keys now use cute slime/bat/skull/mage/wisp/oni/talisman/reaper/kitsune sprites.
...
- Added Playwright regression tests covering the previous branding, BGM loop, early pacing, terrain variation, character frame anchoring, records, and bestiary persistence. `npm.cmd run build` and `npx.cmd playwright test --reporter=line` both pass.
...
- Added GitHub Pages deployment through `.github/workflows/pages.yml` and fixed Vite base-path asset loading so the game can run from `https://suke9800.github.io/phantom-survivor-game/` as well as local dev.
